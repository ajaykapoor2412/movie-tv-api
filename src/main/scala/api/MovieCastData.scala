...
object MovieCastData {
  ...
  def props(ids: List[Entertainment], totalEntertainment: Int) = Props(new MovieCastData(ids, totalEntertainment))

}

class MovieCastData(ids: List[Entertainment], totalEntertainment: Int) extends Actor with ActorLogging {
    ...
    case GetMovieCastData => {
      ...
      def getMoviePageData(listMovieId: List[Entertainment], movieList: Future[List[Cast]]): Future[List[Cast]] = {
        listMovieId match {
          ...
          case x :: tail => {
            movieList.map(x => log.info("MovieCast:- " + x.length + " Movie Count:- " + tail.length))
            val id = x.id
            val httpResponse = Http(context.system).singleRequest(HttpRequest(uri = s"$apiURL/movie/$id/credits?api_key=$apiKey"))
              .flatMap(x =>
                x.status match {
                  case StatusCodes.OK => {
